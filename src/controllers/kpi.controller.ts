...
export const getAllKpi: RequestHandler<any> = asyncWrap(async (_req, res) => {
  try {
    const data = await KpiData.createQueryBuilder('kpi_data')
      .leftJoinAndSelect('kpi_data.allocation', 'kpi_allocation')
      .execute();
...
export const postAllocateRoles: RequestHandler<any> = asyncWrap(
...
export const putUpdateRoles: RequestHandler<any> = asyncWrap(
  async (req: Request, res: Response) => {
    try {
      ...
      if (status) changeUploadedKpiStatus(allocated);
      const update = await KpiAllocation.update(
        { id: allocated!.id },
        { allocated_to_roles: roles, status },
      );
      res.status(205).json({ update, message: 'Updated Successfully' });
    } catch (error) {
      console.error(error);
...
export const getAllocatedKpi: RequestHandler<any> = asyncWrap(
  async (_req, res) => {
    try {
      ...
      for (const allocated of allocatedKpiData) {
        let responseKpi = {};
        const uploadedSheet = await UploadedSheet.findOne({
